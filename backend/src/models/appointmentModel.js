...
const AppointmentModel = {
  getAllAppointments: async (userId = null, filters = {}) => {
    ...
    const joinClause = `
      ...
      LEFT JOIN users unv ON nv.user_id = unv.id
    `;
    ...
    try {
      ...
      for (let row of rows) {
        const [staffRows] = await db.query(
          `SELECT nv.id, u.name, u.phone, nv.chuc_vu
           FROM lich_hen_nhan_vien lhnv
           JOIN nhan_vien nv ON lhnv.nhan_vien_id = nv.id
  ...
  createAppointment: async (data) => {
    ...
    // 1. Nếu có userId, tìm khach_hang_id tương ứng
    if (userId) {
    ...
    const [result] = await db.query(
      `INSERT INTO lich_hen 
       (khach_hang_id, guest_name, guest_phone, nhan_vien_id, goi_id, ngay_bat_dau, ngay_ket_thuc, loai_lich, dia_diem, menu_chon, lich_trinh, dat_coc, loai_phong, trang_thai_thanh_toan, hinh_thuc_thanh_toan, ngay_sinh_be, hinh_thuc_sinh, tinh_trang_me, can_nang_be, ghi_chu_be, dia_chi_cu_the, toa_do, status) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'cho_xac_nhan')`,
      [
        khach_hang_id, 
        guest_name || null, 
    ...
    try {
      const [allStaff] = await db.query("SELECT user_id FROM nhan_vien");
    ...
    try {
        ...
        if (aptRows.length > 0) {
          ...
          if (shiftRows.length === 0) {
            await db.query(
              ...
              [numericId, apt.nhan_vien_id, apt.ngay_bat_dau, "da_nhan"]
            );
          }
      ...
      if (status === "hoan_thanh") {
        ...
        if (aptRows.length > 0) {
          ...
          if (trang_thai_thanh_toan === 'da_thanh_toan_het') {
