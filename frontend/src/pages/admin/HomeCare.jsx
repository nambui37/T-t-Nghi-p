    ...
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Lịch tại nhà từ chờ xác nhận đến đang triển khai (sau khi xác nhận tại
            &quot;Lịch hẹn&quot;, ca hiển thị ở đây).
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-3 md:items-center">
        <input
          ...
          className="px-4 py-2 border border-gray-200 rounded-xl w-full md:max-w-md focus:outline-none focus:ring-2 focus:ring-pink-500"
        />
        <select
          value={filterStatus}
      ...
      {isLoading ? (
        ...
        <div className="py-20 text-center bg-white rounded-3xl border border-dashed border-gray-200">
          <p className="text-gray-500">
            Không có lịch tại nhà trong giai đoạn này hoặc không khớp bộ lọc.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {filteredBySearch.map((apt) => (
            <div
              key={apt.id}
              className="bg-white p-6 rounded-3xl shadow-sm border border-pink-100 hover:shadow-md transition flex flex-col"
            >
              <div className="flex justify-between items-start mb-4 border-b border-gray-50 pb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-pink-50 text-pink-500 flex items-center justify-center font-bold">
              ...
              <div className="space-y-3 text-sm text-gray-600 mb-6 flex-1">
                <div className="flex items-start gap-2 bg-gray-50 p-3 rounded-xl border border-gray-100">
                  ...
                  <span className="font-bold text-gray-800 leading-tight">
                    {apt.dia_chi_cu_the || "Chưa cập nhật địa chỉ cụ thể"}
                  </span>
                </div>
